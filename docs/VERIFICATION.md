...
| SoapySDR plugin over the native SDK | Native SDK | **Verified manually** on Windows 11: built with MSVC (VS 2022 Build Tools) against radioconda's SoapySDR 0.8.1, loads into it, and streams 15.357 MS/s of a 15.36 MS/s request into Python. `find` advertises a `sdk=true` device when the SDK is installed, and `sdk=true`/`serial=` open it; `SoapySDR.Device("driver=aaronia,sdk=true")` works (that build's binding rejects the `dict` form for every driver). The C++-host DLL-load failure — the loader reaching radioconda's own Qt6 first — is fixed in 0.9.0 (search restricted to the DLL dir, SDK root, System32) |
| SoapySDR bandwidth API (`setBandwidth`, `getBandwidth`, `listBandwidths`, `getBandwidthRange`) | Both | **Verified manually** on a V6 ECO over the native SDK: `getBandwidth` at 15.36 MS/s reports 12.288 MHz (0.8x), the ladder lists one usable bandwidth per rung, and `setBandwidth` snaps to a rung through the live-verified `setSampleRate`. Pure reparametrization, so the HTTP path follows |
| SoapySDR sensors (temperatures, ADC range, buffer fill, GPS) | HTTP | **Live-verified** on a V6 ECO: the crate's `live_device_sensors` reads real values over HTTP (FPGA temp, ADC range), and the plugin's `readSensor`/`listSensors` return the full set through `SoapySDRUtil` / Python. Parser also unit-tested against a `/healthstatus`-shaped tree. Over the native SDK the plugin reports only `cumulative_drops`: the raw SDK's `AARTSAAPI_ConfigHealth` tree reads all zeros (dumped live), the live telemetry being RTSA-Suite's, not the raw SDK's — so rich sensors are HTTP-only by design |
| Stream clock-source **write** (`set_clock_source`, `setClockSource`) | Both | **Not yet hardware-verified.** Unit-tested against a `/remoteconfig`-shaped tree: the write lands on the `device` group's `sclksource` leaf carrying the label, is read back to confirm, and a confirmed mismatch is rejected as an error rather than reported as success. A V6 ECO offers `GPS`, `PPS` and `10MHz`, so this is testable — verify before relying on it |
